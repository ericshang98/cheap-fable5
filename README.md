...
```bash
git clone https://github.com/ericshang98/cheap-fable5.git
mkdir -p ~/.claude/skills
cp -r cheap-fable5/skills/cheap-fable5 ~/.claude/skills/
```
...
```
/cheap-fable5 add dark mode to the Android app: settings toggle UI, theme token refactor across 40 files, global color replacement, build verification
```
...
Edit `~/.claude/skills/cheap-fable5/SKILL.md`:
...
**安装**：clone 本仓库，把 `skills/cheap-fable5` 拷到 `~/.claude/skills/`。

**使用**：任意 session 里 `/cheap-fable5 <你的任务>`，建议配合 `/model sonnet` 把主循环也降下来。
